...
type Props = {
  label: string;
  theme?: 'primary' | 'google' | 'facebook' | 'signup';
  onPress?: () => void;
};
...
export default function Button({ label, theme, onPress }: Props) {
  ...
  if (theme === 'google') {
    return (
      ...
      { borderWidth: 3, borderColor: '#C0C0C0', borderRadius: 18 },]}>
        <Pressable
          style={styles.button}
          onPress={onPress || (() => alert('Google login pressed'))}
        >
          <Text style={[styles.buttonLabel, { color: '#25292e' }]}>{label}</Text>
  ...
  if (theme === 'facebook') {
    return (
      ...
      { borderWidth: 3, borderColor: '#C0C0C0', borderRadius: 18 },]}>
        <Pressable
          style={styles.button}
          onPress={onPress || (() => alert('Facebook login pressed'))}
        >
          <Text style={[styles.buttonLabel, { color: '#25292e' }]}>{label}</Text>
  ...
  if (theme === 'signup') {
    return (
      <View style={[styles.buttonContainer,]}>
        <Pressable
          style={styles.button}
          onPress={onPress || (() => alert('Signup pressed'))}
        >
  ...
  return (
    ...
    { borderWidth: 2, borderColor: '#000000', borderRadius: 18 },]}>
      <Pressable
        style={styles.button}
        onPress={onPress || (() => alert('Button pressed'))}
      >
        <Text style={[styles.buttonLabel, { color: '#25292e' }]}>{label}</Text>
      </Pressable>
    </View>
